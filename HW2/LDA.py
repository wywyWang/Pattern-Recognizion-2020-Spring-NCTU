...
def plot_result(x_train, y_train, weight, class_mean, filename):
    fig = plt.figure()
    # plt.xlim(min(x_train[:, 0]), max(x_train[:, 0]))
    # plt.ylim(min(x_train[:, 1]), max(x_train[:, 1]))
    # Plot project line
    project_x = np.linspace(0.01, 0.1, 50)
    ...
    plt.title("Projection line of {}: w = {}, b = {}"
              .format(filename, project_slope[0], project_intercept))
    ...
    for class_idx in range(CLASS):
        ...
        for idx, data in enumerate(match_data):
            ...
            plt.plot([data[0], lower_data[0]], [data[1], lower_data[1]],
                     c='b', alpha=0.1)
    plt.savefig('plotting/Task1_' + filename + '_dimension_reduction.png')


def LDA(x_train, y_train, x_test, y_test, filename):
    ...
    Sm = within_class + between_class
    numerator = np.sum(np.diagonal(Sm))
    denominator = np.sum(np.diagonal(within_class))
    print("Separability measures before projection: {}".format(numerator / denominator))
    ...
    lower_class_mean = compute_mean(lower_dimension_train, y_train)
    lower_within_class = compute_withinclass(lower_dimension_train, y_train, lower_class_mean)
    lower_between_class = compute_betweenclass(lower_class_mean)
    lower_Sm = lower_within_class + lower_between_class
    lower_numerator = lower_Sm
    lower_denominator = lower_within_class
    print("Separability measures after projection: {}".format(lower_numerator / lower_denominator))
    # y_pred = knn(lower_dimension_train, y_train, lower_dimension_test)
    # compute_accuracy(y_pred, y_test)
    plot_result(x_train, y_train, weight, class_mean, filename)
    return lower_dimension_train, lower_dimension_test
