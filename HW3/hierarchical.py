...
def visualization(data, clusters, link_type, data_type):
    ...
    if data_type == 'circles' or data_type == 'moons':
        ...
        for selected_iteration in iterations:
            ...
            fig.suptitle('Scatter Plot for {} link clusters in iteration {}'.format(link_type, selected_iteration+1))
            ...
            plt.savefig('./{}_{}_link/{}.png'.format(data_type, link_type, selected_iteration+1))
            plt.clf()
    else:
...
if __name__ == "__main__":
    ...
    # Data 2
    data_type = 'circles'
    data = make_circles(n_samples=400, factor=0.5, noise=NOISE)
    link_type = 'single'
    before = time.time()
    clusters = clustering(data[0], link_type)
    print("Clusters time: {}".format(time.time() - before))
    ...
    # Data 3
    data_type = 'moons'
    data = make_moons(n_samples=400, noise=NOISE)
    link_type = 'single'
    before = time.time()
    clusters = clustering(data[0], link_type)
    print("Clusters time: {}".format(time.time() - before))
